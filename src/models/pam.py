...
class Connections:
    ...
    def generate_binary_tensor(self):
        ...
        indices = torch.randperm(total_elements)[
            : int(self.synaptic_density * total_elements)
        ]
...
class PamModel:
    ...
    def initialize(self):
        ...
        self.start_sdr = self.create_max_sdr(
            logits=torch.randn((self.N_c * self.N_k, 1))
        )
    ...
    def boost(self, a, bs):
        ...
        a += bs * ((1 / self.N_k) - boost_factor).reshape(a.shape) + torch.normal(
            mean=0.0, std=0.1, size=a.shape
        )
        return a
    ...
    def forward_transition(self, z):
        return SDR.from_nodes_threshold(
            self.transition(z), threshold=self.transition_configs["threshold"] * len(z)
        )
    ...
    def forward_emission(self, z_pred, z_pred_single, iters=100):
        ...
        for i in range(iters):
            ...
            z_pred_single_output = SDR.from_nodes_threshold(
                z_pred_out,
                threshold=self.emission_configs["threshold"] * len(z_pred_single_input),
            )
            z_pred_single_output = z_pred_single_output.intersect(z_pred)

            if z_pred_single_input == z_pred_single_output:
                break
    ...
    def learn_sequence(self, sequence):
        ...
        for p in sequence[1:]:
            ...
            # train transition
            a = self.transition(z)
            z_new = p.expand(self.N_k).intersect(
                self.create_max_sdr(self.threshold_a(a))
            )

            for i in range(1000):
                self.transition.train_A(z, z_new)
                if self.evaluate_transition(z, p):
                    break

            # train emission
            z_pred = SDR.from_nodes_threshold(
                self.transition(z),
                threshold=self.transition_configs["threshold"] * len(z),
            ).reduce(self.N_k)
            for i in range(1000):
                self.emission.train_B(z_pred, p)
                if self.evaluate_emission(z_pred, p):
                    break
    ...
    def recall_sequence_online(self, sequence):
        ...
        for p in sequence[1:]:
            ...
            z_pred = SDR.from_nodes_threshold(
                a, threshold=self.transition_configs["threshold"] * len(z)
            )
    ...
    def generate_sequence_online(self, sequence):
        ...
        for p in sequence[1:]:
            ...
            if len(z_pred_single) == 0:
                z_pred_single = p.choose(self.W)
    ...
    def generate_sequence_offline(self, x_in_sdr, seq_len):
        ...
        for _ in range(seq_len):
            ...
            for i in range(100):
                ...
                if len(z_pred_single) == self.W:
                    break

            if len(z_pred_single) == 0:
                z_pred_single = z_pred.choose(self.W)
            if len(z_pred_single) > self.W:
                z_pred_single = z_pred_single.choose(self.W)
    ...
    def save(self, path=None):
        ...
        for p in self.parameters:
            attr = getattr(self, p)
            to_save[p] = (
                attr if isinstance(attr, torch.Tensor) else attr.save(path=None)
            )
