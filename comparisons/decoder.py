...
embs = nn.Embedding(10, 512, _freeze=True)
...
def test():
    ...
    inputs = embs.weight.data + torch.normal(mean=0, std=5.0, size=embs.weight.data.size())
    preds = model(inputs)
    loss = F.mse_loss(preds, labels)
